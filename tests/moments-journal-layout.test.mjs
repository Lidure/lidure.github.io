...
test('Moments enhancement uses a wide three-column notebook canvas with compact side gutters', () => {
  ...
  assert.match(notebookCss, /\.moments-shell\s*\{[\s\S]*width:\s*min\(100%,\s*112rem\)\s*!important[\s\S]*max-width:\s*112rem\s*!important/);
  assert.match(notebookCss, /\.moments-page-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(12\.5rem,\s*14rem\)\s+minmax\(0,\s*1fr\)\s+minmax\(12\.5rem,\s*14rem\)[\s\S]*gap:\s*clamp\(1rem,\s*1\.4vw,\s*1\.5rem\)/);
  assert.match(notebookCss, /\.moments-main-column\s*\{[\s\S]*width:\s*100%/);
  assert.match(notebookCss, /\.moment-card\s*\{[\s\S]*width:\s*100%\s*!important[\s\S]*max-width:\s*none\s*!important/);
  assert.match(notebookCss, /\.moment--photo-one[\s\S]*\.moment--text\s*\{[\s\S]*max-width:\s*none\s*!important/);
...
test('Moments snapshots map thumbnails to every rendered image type and open the existing lightbox', () => {
  assert.match(baseLayout, /MomentsSnapshotPreviewBridge/);
  assert.match(pins, /\.card-images img, \.moment-media img/);
  assert.match(pins, /button\.dataset\.snapshotIndex\s*=\s*String\(index\)/);
  assert.doesNotMatch(pins, /matchingImage/);
  assert.doesNotMatch(pins, /currentSrc\s*===/);
  assert.match(snapshotBridge, /\.moments-side-photo/);
  assert.match(snapshotBridge, /dataset\.snapshotIndex/);
