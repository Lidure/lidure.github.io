...
export async function commitGitHubUploadTransaction({
  ...
  if (!owner || !repo || typeof request !== 'function') throw new Error('GitHub 上传事务配置不完整');
  if (!Array.isArray(items) || !items.length) throw new Error('没有可上传的图片');
  if (!manifest?.path || typeof manifest.contentBase64 !== 'string') throw new Error('感知查重索引内容缺失');
  for (const item of items) {
    if (Number(item.size) > GITHUB_MAX_BLOB_BYTES) throw new Error(`${item.path} 超过 GitHub 单文件 100 MiB 限制`);
    if (!item.path || (typeof item.loadContentBase64 !== 'function' && typeof item.createBlob !== 'function')) {
  ...
  async function readBase() {
    ...
    if (treeResult?.data?.truncated) throw new Error('GitHub 文件树被截断，为避免覆盖已拒绝上传');
    if (!Array.isArray(treeResult?.data?.tree)) throw new Error('GitHub 文件树响应无效，为避免覆盖已拒绝上传');
    const tree = treeResult.data.tree;
    for (const item of items) {
      const plannedNumber = globalGalleryImageNumber(item.path);
      if (plannedNumber && tree.some(entry => entry?.type === 'blob' && globalGalleryImageNumber(entry.path) === plannedNumber)) {
        throw transactionError(`全局编号已存在：${plannedNumber}`, 'GLOBAL_NUMBER_CONFLICT');
      }
      ...
      if (item.expectedBlobSha && tree.some(entry => (
        entry?.type === 'blob' && entry.sha === item.expectedBlobSha && String(entry.path || '').startsWith(prefix)
      ))) {
  ...
  const objects = [...items, { path: manifest.path, loadContentBase64: async () => manifest.contentBase64 }];
  let completed = 0;
    ...
    if (typeof object.createBlob === 'function') {
      ...
      try {
        const result = await safeObjectRequest(
          request, 'POST', `${api}/git/blobs`, { body: { content, encoding: 'base64' } }, sleep,
        );
        sha = result?.data?.sha || '';
      } finally {
        content = null;
  ...
  async function createCommit(currentBase) {
    const treeResult = await safeObjectRequest(
      request, 'POST', `${api}/git/trees`, { body: { base_tree: currentBase.treeSha, tree: blobEntries } }, sleep,
    );
  ...
  async function verifyUncertainRef(commitSha) {
    try {
      ...
      const treeResult = await safeObjectRequest(
        request, 'GET', `${api}/git/trees/${currentTree}`, { params: { recursive: '1' } }, sleep,
      );
  ...
  try {
    ...
    if (REF_CONFLICT_STATUSES.has(Number(error?.status))) {
      ...
      try {
        ...
        if (transientRequestError(retryError) && await verifyUncertainRef(commitSha)) return { commitSha, entries: blobEntries };
        throw retryError;
      }
