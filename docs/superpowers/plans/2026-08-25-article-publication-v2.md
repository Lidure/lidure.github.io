...
**Goal:** Replace the PR #43 editorial-card article detail page with a bannerless, authored personal-publication layout that prioritizes Chinese long-form reading, asymmetric rich content, and a minimal chapter bookmark rail.

**Architecture:** Add one narrow `showBanner?: boolean` capability to `BaseLayout`, then rebuild `src/pages/posts/[slug].astro` around a masthead + reading canvas instead of a dominant card. Replace `article-reading.css` wholesale, keep `estimateReadingMinutes`, render H2 bookmark ticks from Astro `headings`, and use one small page-local controller that updates reading position without touching wallpaper state.
...
- Article detail and Moments must eventually use `showBanner={false}`; this plan wires the article page only.
- Keep global site header, HeroSlideshow wallpaper source, particles, player, visual settings, page transitions, standard content surface, and footer.
...
- Do not add AI summaries, a TOC card, Mac traffic-light code decoration, or new required frontmatter.
...
### Task 1: Add the exact banner opt-out and article page contract
...
**Interfaces:**
- Consumes: existing `BaseLayout` standard-mode rendering.
- Produces: `showBanner?: boolean` prop with default `true`; article passes `showBanner={false}`.

- [ ] **Step 1: Replace the old article source regression test with a V2 contract test**

Replace the first article-page test in `tests/article-reading.test.mjs` with:
...
test('article detail owns its page entrance instead of the standard banner', () => {
...
Expected: FAIL because `BaseLayout` does not expose `showBanner` and the article still contains PR #43 markup.

- [ ] **Step 3: Add `showBanner` to `BaseLayout` exactly once**
...
Change only the standard banner stage from:
...
Do not conditionally remove `.standard-page-surface` or the footer.

- [ ] **Step 4: Make the article opt out**

Add the prop to the existing `BaseLayout` call:

```astro
showBanner={false}
```

Do not change the title/description SEO props.

- [ ] **Step 5: Commit the shared layout capability**

Run the focused test. It is allowed to remain RED on the old article-markup assertions until Task 2, but verify the failure is now only the retired article markup.
...
### Task 2: Rebuild article masthead and semantic reading structure
...
**Interfaces:**
- Consumes: `post.data.title`, `post.data.description`, `post.data.pubDate`, `post.data.updatedDate`, `post.data.tags`, `post.body`, Astro `render(post)`.
...
```js
test('article uses the personal-publication structure', () => {
...
Expected: FAIL on the new V2 classes/headings contract.

- [ ] **Step 3: Replace the page frontmatter preparation**
...
Delete `issueYear` and the PR #43 presentation-only values.

- [ ] **Step 4: Replace the article markup wholesale**

Inside `BaseLayout`, replace the current `<article class="post-shell">...</article>` with:
...
Do not add a visual comments heading.

- [ ] **Step 5: Run focused test and verify GREEN for structure**

```bash
node --test tests/article-reading.test.mjs
```

Expected: the structure tests pass; later style/controller tests may still fail if already added.
...
### Task 3: Replace article styling with the authored publication visual language
...
**Interfaces:**
- Consumes: V2 markup from Task 2 and existing `--standard-*`, `--card-opacity-percent`, wallpaper-mode data attributes.
- Produces: 1160px publication canvas, 720px reading column, CSS H2 counters, asymmetric breakout media/code/table/quote treatment, mobile flattening.
...
Expected: FAIL because the old PR #43 card stylesheet is still present.

- [ ] **Step 3: Replace `src/styles/article-reading.css` with this complete base**
...
body.layout-standard .article-prose h3 { margin: 2.25em 0 0.72em; font-size: 1.2rem; line-height: 1.42; }
body.layout-standard .article-prose h4 { margin: 1.8em 0 0.58em; font-size: 1.02rem; line-height: 1.48; }
...
body.layout-standard .article-prose hr {
  width: 42px;
  margin: 3.4em auto;
  border: 0;
  border-top: 1px solid color-mix(in srgb, var(--standard-line) 78%, transparent);
}
...
html[data-wallpaper-mode="overlay"] body.layout-standard .article-publication {
  background: linear-gradient(
    to bottom,
    color-mix(in srgb, var(--standard-card-bg) calc(var(--card-opacity-percent, 92%) * 0.42), transparent),
    color-mix(in srgb, var(--standard-card-bg) calc(var(--card-opacity-percent, 92%) * 0.26), transparent)
  );
}
...
### Task 4: Implement the bookmark rail and mobile reading progress
...
**Interfaces:**
- Consumes: `.article-prose`, `.article-bookmark-tick[data-article-chapter]`, heading IDs.
- Produces: `--article-reading-progress` on `.article-publication`; `--chapter-offset` on each tick.
...
```js
test('article bookmark rail follows real headings and cleans up across Astro navigation', () => {
  assert.match(page, /data-article-chapter/);
...
Expected: FAIL because the PR #43 controller only tracks generic progress.

- [ ] **Step 3: Replace the page script with a rail-aware controller**
...
- [ ] **Step 4: Append exact bookmark CSS**
...
### Task 5: Integration regression and final article verification

**Files:**
- Modify if needed: `tests/article-reading.test.mjs`
- Modify if needed: `package.json`

**Interfaces:**
- Consumes: all article tasks.
- Produces: green site tests/build and no PR #43 article visual hooks.

- [ ] **Step 1: Add the retirement regression**

Append:

```js
test('retired PR #43 article UI does not return', () => {
  assert.doesNotMatch(page, /article-issue-stamp/);
  assert.doesNotMatch(page, /article-meta-rail/);
  assert.doesNotMatch(page, /article-comments-label/);
  assert.doesNotMatch(page, /class="post-shell"/);
  assert.doesNotMatch(css, /h2::before\s*\{[\s\S]*box-shadow:/);
});
```

Keep `tests/article-reading.test.mjs` in `test:site`; do not add a duplicate test entry.

- [ ] **Step 2: Run focused and full static tests**
...
- [ ] **Step 3: Run Astro validation/build**

```bash
npm run build
```

Expected: `astro check` and `astro build` both PASS. Confirm at least one article with KaTeX still builds.

- [ ] **Step 4: Review the diff specifically for scope leaks**
...
- `showBanner` defaults to `true`;
- no homepage/list/archive behavior changed;
- no wallpaper variables are written by article JS;
- no required frontmatter was added;
- comments and KaTeX imports remain.

- [ ] **Step 5: Commit any final test-only corrections**

If verification required no code correction, do not create an empty commit. Otherwise:

```bash
git add src/layouts/BaseLayout.astro src/pages/posts/[slug].astro src/styles/article-reading.css tests/article-reading.test.mjs package.json
